...
double Point::Distance(const Point& p, DistanceStrategy* strategy = new ExactDistance) {
    return strategy->Distance(std::ref(*this), std::ref(p));
}

// distance from origin for specific strategy
double Point::Distance(DistanceStrategy* strategy = new ExactDistance) {
    return strategy->Distance(std::ref(*this), std::ref(*OriginPoint::instance()));
}
