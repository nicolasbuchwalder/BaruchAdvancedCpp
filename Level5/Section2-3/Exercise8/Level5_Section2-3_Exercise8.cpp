// Level5_Section2-3_Exercise8.cpp: This file contains the full code for this exercise
...
ResultSet read_csv(const std::string& filename) {
	...
	// if opened correctly
		...
		while (std::getline(file, line)) {
			...
			// defining regex and regex iterators
			std::regex reg(",");
			auto begin_it = std::sregex_token_iterator(line.begin(), line.end(), reg, -1);
			auto end_it = std::sregex_token_iterator();

			// splitting row in elements by "," seperator
			std::vector<std::string> row;
			for (auto it{ begin_it }; it != end_it; ++it) { row.push_back(*it); };
